...
func (h *Handler) Handle(req handleRequest) types.Response {
	...
	if err != nil {
		log.Debugf("debug on test:: B\n")
		return types.NewResponse(req.Request, nil, err)
	}
	log.Debugf("debug on test:: C\n")

	inArgsOffset := 0
	inArgs := make([]reflect.Value, fd.inNum)
	inArgs[0] = service.sv
	log.Debugf("debug on test:: D\n")
	...
	if fd.numParams() > 0 {
		log.Debugf("debug on test:: I\n")
		if err := json.Unmarshal(req.Params, &inputs); err != nil {
			log.Debugf("debug on test:: Ierr\n")
	...
	log.Debugf("debug on test:: K\n")
	var data []byte
	res := output[0].Interface()
	if res != nil {
		log.Debugf("debug on test:: L\n")
		d, _ := json.Marshal(res)
		log.Debugf("result data => %v\n", string(d))
		data = d
	}
